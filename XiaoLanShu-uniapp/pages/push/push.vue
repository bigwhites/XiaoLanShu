<template>
	<view class="container">
		...
		<view class="main">
			...
			<view class="content">
				<view>
					<textarea placeholder="标题" maxlength="30" class="input-title" auto-height=false
						style="height:30rpx;" v-model="imgInfo.title"></textarea>
				</view>
				<view>
					<textarea placeholder="内容" maxlength="30000" class="content-input" auto-height=false
	...
	export default {
		...
		methods: {
			...
			chooseImg() {
				...
				uni.chooseImage({
					...
					success: function(res) {
						...
						uni.setStorageSync("imgList", JSON.stringify(that.imgList)) //图片
					}
				})
			...
			next() {
				...
				let userId = uni.getStorageSync("userBasic").uuid
				...
				if (false) {
					...
					that.imgInfo.content = this.imgInfo.content;
					that.imgInfo.pubUuid = userId
					that.imgInfo.tags = that.tagList;
					that.imgInfo.title = this.imgInfo.title;
					uni.setStorageSync('imgInfo', JSON.stringify(that.imgInfo))

					uni.navigateTo({
						url: '/pages/addalbum/addalbum?type=' + 'add'
					})
				}
			},
