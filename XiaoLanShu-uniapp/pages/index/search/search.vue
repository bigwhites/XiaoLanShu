		...
		<view class="search-list" v-if='cursor'>
			<view v-if="userList.length>0">
				<view class="tui-item-box" v-for="(user,index) in userList" :key="index">
					<view class="tui-msg-box" @click="getUserInfo(user.uuid,user.isFollow)">
						<image :src="user.uAvatar" class="tui-msg-pic" mode="aspectFill"></image>
						<view class="tui-msg-item">
							<view class="tui-msg-name">{{user.nickname}}</view>
							<view class="tui-msg-type">
								<view>ID：{{ user.userName }}</view>
	...
	export default {
		...
		methods: {
			...
			change() {
				...
				this.cursor = true
				const uuid = uni.getStorageSync('userBasic').uuid;
				let params = {
					uNameOrNick: this.keyword,
					uuid: uuid
				}
				searchUsers(params).then(
					res => {
				...
				// esSearchRecord(params).then(res => {  //搜索文章
			...
			getUserInfo(uid, isFollow) { //跳转到其他用户的界面
				if (uid == uni.getStorageSync("userInfo").id) {
					...
					uni.navigateTo({
						url: "/pages/otherUser/otherUser?uid=" + uid + '&isFollow=' + isFollow
					})
				}
			},
