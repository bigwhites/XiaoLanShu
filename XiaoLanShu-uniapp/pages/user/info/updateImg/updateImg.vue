...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.uid = uni.getStorageSync('userBasic').uuid;
		...
		methods: {
			...
			publish() {
				...
				let that = this
				that.disabled = true
				reqUploadFile(this.uid, 'a.png', this.type).then( res =>{
					 console.log(`res:${res.data.pathKey}`)
					uni.uploadFile({
						url: appConfig.STATIC_PATH + 'fileUpload/one',
						filePath: this.value,
						name: 'file',
						header: {
							uploadtoken: res.data.pathKey,
							nametoken: res.data.nameKey,
						},
						success: (res) => {
							uni,
							uni.showToast({
