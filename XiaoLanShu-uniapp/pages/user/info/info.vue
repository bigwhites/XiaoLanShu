<template>
	<view class="content">
		...
		<view class="body">
			<view class="body-img">
				<image :src="userInfo.uAvatar" mode="aspectFill" />
				<view class="body-rz">
			...
			<view class="center">
				<view class="avatar">
					<image :src="userInfo.cover" mode="aspectFill"></image>
				</view>
				<view class="center-button">
					<button @click="updateImg('avatar')">更换头像</button>
					<button @click="updateImg('cover')">更换封面</button>
				</view>
			</view>
		</view>
			...
			<ul>
				...
				<li class="info-item">
					<view class="info-item-zhuti" @click="updateInfo(2)">
						...
						<view class="cont">{{ userInfo.uSex}}</view>
						<view class="icon"><tui-icon name="arrowright" size="25"></tui-icon></view>
					</view>
				...
				<li class="info-item">
					<view class="info-item-zhuti" @click="updateInfo(4)">
						...
						<view class="cont">{{ userInfo.uAbout }}</view>
						<view class="icon"><tui-icon name="arrowright" size="25"></tui-icon></view>
					</view>
				</li>
				<li class="info-item">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.getUser(uni.getStorageSync('userBasic').uuid) //更新用户信息只有可能是自己
		...
		onShow(options) {
			...
			// this.coverUrl = uni.getStorageSync("aVatarUrl");
			// this.aVatarUrl = uni.getStorageSync("coverUrl");
		},
		created() {
		...
		methods: {
			...
			updateInfo(index) {
				if (index == 1 || index == 4) {
					//修改昵称和简介
					uni.navigateTo({
						url: "/pages/user/info/updateInfo/updateInfo?type=" + index //1是昵称，4是简介
					})
				} else if (index == 2) {
					//更改性别
					this.sexShow = true
