	...
	<view class="content">
		<tui-navigation-bar backgroundColor="#fff" :isFixed="true" :isOpacity="false">
			<view class="nav">
				...
				<view class="center">
					<view @click="getUserInfo(imgInfo.nickname)">
						<image class="avatar" :src="imgInfo.uAvatar" mode="aspectFill" />
					</view>

					<view class="username">{{ imgInfo.nickname }}</view>
		...
		<scroll-view scroll-y class="page" @scrolltolower="loadData" @scroll="scroll" :scroll-top="scrollTop">
			...
			<scroll-view scroll-x="true" @scroll="scroll" scroll-left="10" styele="white-space: nowrap; ">
				<view v-for="(item, index) in imgs" :key="index">
					<view>
						<img :src="item" mode="scaleToFill" height="700px" width="100%" style="
						display: inline-block;line-height: 300rpx;text-align: center;" @click="previewImgae(index)"></img>
					</view>
				</view>
			</scroll-view>
			...
			<view class="cont">
				<view class="img-content">
					<h2>{{ imgInfo.basicInfo.title }}</h2>
				</view>
				<view style="min-height: 20px;"></view>
				<view class="img-content">{{ imgInfo.basicInfo.content }}</view>
				<view class="time">{{Timeago(imgInfo.basicInfo.publishTime) }}</view>
				<view>{{ imgInfo.basicInfo.viewCount }}人看过</view>
			</view>
			<view class="relation">
				<!-- 	<view class="r-all">
					<h4>所属专辑</h4>
		...
		<view :class="fn?'fotter fotter-none':'fotter'">
			...
			<view v-if="!useful_flag" class="fotter-info">
				...
				<uv-icon name="heart-fill" v-if="isAgree==true" size="20" @click="agreeNote()"
					:label="agreeCount"></uv-icon>
				<uv-icon v-else name="heart" size="20" :label="agreeCount" @click="agreeNote()"></uv-icon>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.imgInfo.blogId = option.blogId
		...
		methods: {
			...
			agreeNote() {
				let that = this;
				changeAgree(this.imgInfo.basicInfo.id, uni.getStorageSync('userBasic').uuid).then(res => {
					if (res.success) {
			...
			previewImgae(index) {
				...
				uni.previewImage({
					...
					urls: that.imgs, // 需要预览的图片列表，photoList要求必须是数组
					longPressActions: {
						itemList: ['保存', '收藏'],
						success: function(data) {
							...
							//进行保存
							if (data.tapIndex == 0) {
								let p = that.downLoadImg(that.imgs[data.index])
								p.then((data) => {
									uni.showToast({
										title: data,
										icon: "none"
			...
			getOne() {
				getOneNoteById(this.imgInfo.blogId, uni.getStorageSync('userBasic').uuid).then(res => {
					if (res.success) {
